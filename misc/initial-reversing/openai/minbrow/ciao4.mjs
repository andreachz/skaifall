...
// --- 2) Set up jsdom and globals --------------------------------------------
const dom = new JSDOM(`<!doctype html><html><body></body></html>`, {
  url: 'https://chatgpt.com/',
  pretendToBeVisual: true,
  runScripts: 'outside-only', // we'll run scripts ourselves
});

// minimal globals many browser bundles expect
Object.assign(globalThis, {
  window: dom.window,
  document: dom.window.document,
  self: dom.window,
  location: dom.window.location,
  // Node 22 has fetch/webcrypto already, but just in case:
  // crypto: globalThis.crypto ?? _webcrypto,
});

// navigator is a getter in Node; define a getter that returns the jsdom one
try {
...
// extremely minimal ResizeObserver shim (only presence)
globalThis.ResizeObserver ??= class { observe(){} unobserve(){} disconnect(){} };

// --- 3) Helpers --------------------------------------------------------------
function looksLikeESM(source) {
  // quick heuristic: top-level import/export
  return /^\s*(import\s|export\s)/m.test(source);
}

async function loadClassicScript(source, filenameForStack = 'remote.js') {
  vm.runInContext(source, dom.getInternalVMContext(), { filename: filenameForStack });
}

async function loadESModule(source, baseName = 'remote') {
  // Write to temp with .mjs so Node treats it as ESM
  const filePath = join(tmpdir(), `${baseName}-${Math.random().toString(36).slice(2)}.mjs`);
  await writeFile(filePath, source, 'utf8');
  const fileURL = pathToFileURL(filePath).href;
  // Node executes this in the Node realm, but it can still read globals we set on globalThis
...
for (const url of urls) {
  try {
    ...
    if (looksLikeESM(code)) {
      await loadESModule(code, encodeURIComponent(url.split('/').pop() ?? 'mod'));
      results.push({ url, ok: true, type: 'esm' });
      console.log('✅ ESM loaded:', url);
    } else {
      await loadClassicScript(code, url);
      results.push({ url, ok: true, type: 'classic' });
      console.log('✅ Classic loaded:', url);
    }
  } catch (err) {
...
console.log('First 50 window keys:', Object.keys(window).slice(0, 50));

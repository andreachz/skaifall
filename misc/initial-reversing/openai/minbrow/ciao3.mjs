...
const dom = new JSDOM(`<!doctype html><html><body></body></html>`, {
  ...
  runScripts: "dangerously", // we run code ourselves via vm
});

// Expose minimum globals
...
// Node 22 has webcrypto; expose it like browsers do
...
// very naive ResizeObserver shim (many libs only check for existence)
...
// --- 3) LOADER ---------------------------------------------------------------
function isProbablyESM(source) {
  // crude heuristic: top-level 'import ' or 'export ' tokens
...
async function loadAndRun(url) {
  try {
    ...
    if (isProbablyESM(code)) {
      // vm cannot execute ESM syntax. We’ll skip and mark as unsupported.
      return { url, ok: false, reason: "ESM module syntax not supported in vm" };
    }

    vm.runInContext(code, dom.getInternalVMContext(), { filename: url });
    return { url, ok: true };
  } catch (err) {
    return { url, ok: false, reason: String(err?.message || err) };
  }
}

const results = [];
for (const url of urls) {
  const r = await loadAndRun(url);
  console.log((r.ok ? "✅ Loaded" : "❌ Failed"), url, r.ok ? "" : `→ ${r.reason}`);
  results.push(r);
}

console.log("\n--- SUMMARY ---");
const okCount = results.filter(r => r.ok).length;
console.log(`Loaded: ${okCount}/${results.length}`);
console.log("Window keys sample:", Object.keys(window).slice(0, 50));

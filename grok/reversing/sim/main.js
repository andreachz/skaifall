...
// Cache promise so module is only initialized once
...
async function loadModule(name, method) {
  if (!modulePromise) {
    ...
    const modulePath = pathToFileURL(
      path.resolve("static/chunks/84383766740feaa1.js")
    ).href;

    // Dynamically import once
    modulePromise = import(modulePath).then((mod) => {
      // If it has a default export, return that
      return mod.default || mod;
    });
  }

  const moduleFn = await modulePromise;

  if (typeof moduleFn !== "function") {
    throw new Error("Loaded module does not export a callable function.");
  }

  // Call with arguments
  return await moduleFn(name, method);
}

// Example usage:
(async () => {
  try {
    const result = await loadModule("testName", "testMethod");
...
console.log(prepareRequest(req.e).then(x=>{console.log(x)}))
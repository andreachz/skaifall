...
const { getRandomName, getDateYearsAgo } = require('../../../helpers/names');
...
async function registerNewUser(password='international'){

  password = password || generateUrlSafeString(14) || crypto.randomUUID()
...
// console.log(await jar.getCookies('https://chatgpt.com/auth/login'))
  ...
  console.log('response /log-in-or-create-account', 
    // await res_login.text(), 
    res_login.ok, res_login.status, 
    res_login.statusText)
  ...
  console.log(
    // await res_register.text(),
   res_register.ok, res_register.status, res_register.statusText);
  ...
  console.log('response /send', 
    await res_otp_send.text(), 
  res_otp_send.ok, res_otp_send.status, res_otp_send.statusText);
  ...
  console.log('response /email-verification:', 
    // await res_email_ver.text(), 
  res_email_ver.ok, res_email_ver.status, res_email_ver.statusText);
  ...
  console.log('response /validate:', 
    await res_validate.text(), 
    res_validate.ok, res_validate.status, res_validate.statusText);

  // create account
  const oaisentinel_oauth_create_account = await getOaiSentinelToken('oauth_create_account');

  let rname = getRandomName()
  let bdate = getDateYearsAgo(30)
  ...
  console.log('response /auth?audience',
    //  auth_body, 
    res_auth.ok, res_auth.status, res_auth.statusText);
  ...
  console.log('response /consent?consent', 
    // await res_consent.text(), 
    res_consent.ok, res_consent.status, res_consent.statusText);
  ...
  if (res_oai_code.headers.get('location')) {
    ...
    let sessionCookies = await jar.getCookieString('https://chatgpt.com')
    console.log(sessionCookies)
    // console.log(accessToken)
    let tempmail = getTempMailCreds()
    const result = {accessToken: accessToken, sessionCookies: sessionCookies, username: email, password, name: rname, bdate, tempmail}
    console.log(result)
    return result
  }
  else{
...
async function loginUser(username, password='international'){
...
// console.log(await jar.getCookies('https://chatgpt.com/auth/login'))
  ...
  const oaisentinel_login_password = await getOaiSentinelToken('login_password');


  // res_verify
  let res_verify = await fetch('https://auth.openai.com/api/accounts/password/verify', {
    headers: {
      ...baseHeaders,
      accept: 'application/json',
      'cache-control': 'no-cache',
      'content-type': 'application/json',
      'openai-sentinel-token': JSON.stringify(oaisentinel_login_password),
      pragma: 'no-cache',
      priority: 'u=1, i',
      'sec-fetch-dest': 'empty',
      'sec-fetch-mode': 'cors',
  ...
  console.log(
    // await res_verify.text(),
   res_verify.ok, res_verify.status, res_verify.statusText);



  let res_verify_j = await res_verify.json();
  console.log(
    'response /verify',
    res_verify_j,
    res_verify.ok,
    res_verify.status,
    res_verify.statusText
  );

  // follow the continue_url (authorization)
  let res_auth = await fetch(res_verify_j.continue_url, {
    headers: {
      ...baseHeaders,
      accept:
        'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
  ...
  if (res_oai_code.headers.get('location')) {
    ...
    console.log(sessionCookies)
    // console.log(accessToken)
    const result = {accessToken: accessToken, sessionCookies: sessionCookies, username: email, password}
    return result
  }
  else{
    return null
...
function setCookieArrayToHeader(setCookieArray) {
  return setCookieArray
    .map(cookieStr => cookieStr
      // Remove the leading `Cookie="` if present
      .replace(/^Cookie="/, "")
      // Remove the trailing quote if present
      .replace(/"$/, "")
      // Split into parts (main pair + attributes)
      .split(";")[0]
      .trim()
    )
    .join("; ");
...
// // Example usage:
// const rawCookies = [
//   `Cookie="__Host-next-auth.csrf-token=dbd8ef63d603dee05afa82a2da68c72b4b81646dd9ee11390e0b24f759473993%7C91ff7bf13059be5b33d835b7be6e01a69c0e59bc7d93b38f17b12fc7e61b1e31; Path=/; Secure; HttpOnly; SameSite=Lax; hostOnly=true; aAge=0ms; cAge=8897ms"`,
//   `Cookie="__Secure-next-auth.callback-url=https%3A%2F%2Fchatgpt.com%2F; Path=/; Secure; HttpOnly; SameSite=Lax; hostOnly=true; aAge=0ms; cAge=8897ms"`,
//   `Cookie="oai-did=e5553555-2e74-439f-9fa7-d8f3f99b46e9; Max-Age=31104000; Domain=chatgpt.com; Path=/; SameSite=Lax; hostOnly=false; aAge=0ms; cAge=8897ms"`,
//   // ... etc
// ];

// const cookieHeader = setCookieArrayToHeader(rawCookies);

// console.log(cookieHeader);



// loginUser('lelaw21026@kwifa.com','international')

...
async function getDuckChatHash({ maxAttempts = 15, requestOptions = {} } = {}) {
  ...
  while (attempt < maxAttempts) {
    ...
    let proxies = await getProxyCacheList()
          let url = `https://${defaultOptions.hostname}${defaultOptions.path}`
          headers = Object.fromEntries((await (await raceProxies(proxies, url, defaultOptions, { 
            repeats: 1, 
            spacingMs: 10, 
            maxProxies: 100,
            isWinnerDuckDuckGo 
        ...
        if (evt && typeof evt === "object") {
          if (evt.action === "error") {
            ...
            if (typeof onChunk === "function") {
              let msg = '__DATA: '+evt.message
              onChunk(msg, evt); // stream to caller
            }
          }
        }
      } catch {
...
const {isJson} = require('../../helpers/generic');
const { raceProxies, isWinner, isWinnerDuckDuckGo } = require('../../proxy/proxy');
